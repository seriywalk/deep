output "external_v4_endpoint" {
  ...
  value = yandex_kubernetes_cluster.my_kuber.master[0].external_v4_endpoint
}
...
output "cluster_ca_certificate" {
  ...
  value = yandex_kubernetes_cluster.my_kuber.master[0].cluster_ca_certificate
}

output "cluster_id" {
  description = "ID of a new Kubernetes cluster."

  value = yandex_kubernetes_cluster.my_kuber.id
}
